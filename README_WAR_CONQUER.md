...
4. Elige **1, 2, 3 o 4 personas**. Una persona juega contra una IA; las otras opciones son partidas locales con ese número de personas, sin rellenar puestos con IA. Elige Zukgrok, Sahria o Faunar para cada participante, incluido el rival automático. Cada mazo conserva sus 50 cartas.
...
- Abajo: mano de seis cartas por página, coste/vida arriba, MOV abajo a la izquierda y ATK abajo a la derecha y una banda destacada con la etapa de uso. Las cartas no disponibles se oscurecen más, manteniendo legible la etapa y disponible su consulta. Hover abre una carta ampliada con todos los biomas y etiquetas sin modificar el estado. En pantalla táctil, tocar abre el detalle y permite seleccionar objetivos desde allí.
...
| `Resources/WarConquer/cards.json` | 77 diseños de mazo, 5 genéricas fuera de los mazos y 3 fichas, editables |
...
## Faunar y cartas genéricas

Faunar está disponible para cada puesto humano o IA. Usa el mismo tablero, estado, turnos, coste y reglas de muerte que los otros Líderes. La primera estructura jugada en su turno cuesta 1 menos (mínimo 0); `firstStructureUsed` se consume solamente al pagar una estructura válida y se reinicia una vez al comenzar el turno. La ficha del Líder muestra AVAILABLE o USED.

El mazo contiene exactamente **50 cartas**: 15 Spells, 5 Combat Spells, 18 Structures y 12 Units. Sus 19 diseños respetan la lista solicitada, con un máximo de tres copias y sin Slashing Bear. Clearing Space, Cleansing Conquest, From the Ashes, Engage y Med Camp existen en el catálogo y el motor, con cantidad cero: no se añadieron a ninguno de los tres mazos cerrados. Rabbit Token tampoco ocupa espacio del mazo.

`GenericCardRules` integra las nuevas operaciones con `GameManager`, `EffectManager`, `AbilityManager`, `CombatManager` y `TerrainManager`. No hay un tablero ni un sistema de energía alternativo. `BoardManager.SlabsWithinRadius`, `UnitsWithinRadius` y `StructuresWithinRadius` reutilizan la distancia axial hexagonal, independiente de MOV y de las rutas transitables. Slab = HexTile; Yield = Wasteland/Yermo; Forest = Forest/Bosque.

`ChoiceManager` serializa y valida las elecciones pendientes. One for the Team elige un descarte antes de robar; A Price to Pay roba antes de elegir dos descartes. Emergency Seal y Panther permiten elegir objetivos en el tablero y ceden la prioridad al propietario del efecto. Durante una elección no se puede avanzar de etapa, jugar otras cartas ni resolver el ataque pendiente. La IA resuelve sus propias elecciones. Guardar/cargar conserva la elección pendiente y no vuelve a robar.

Las cartas conservan el formato MOV/HP/ATK. Rabbit calcula +2 HP desde su base solamente mientras el Slab sea Bosque, conservando el daño y sin acumular el bono. Valiant Seal y Courage Seal son auras que se retiran al destruir la estructura. Time to Move aplica +4 MOV a todas las unidades dentro de 8rad y lo retira al terminar el turno actual, incluidos otros jugadores. Bomb aplica exactamente 3 de daño directo a una estructura. Engage usa el ATK actual y requiere un destino libre para desplazar al aliado tras la muerte.

Decisiones de graybox para datos no especificados en el encargo: Berry Bush tiene 3 HP; su Rabbit aparece en el primer Slab adyacente libre y legal, y no aparece si no hay sitio. “Heal me” restaura hasta HP máximo. Med Camp, Resting Camp y Vigilant Tower usan activación una vez por turno en Asalto, siguiendo el sistema de habilidades actual. Una Forest Structure es aliada y tiene etiqueta BOSQUE o está sobre un Slab Bosque. El radio de Panther se mide desde su Slab de llegada. Cleansing Conquest convierte el Slab elegido en Yermo y permite mover una unidad aliada a 8rad de ese destino. Las Spells normales nuevas se pueden usar en las fases principales propias; las Combat Spells también disponen de las ventanas de respuesta de batalla.

Pruebas nuevas: `FaunarRulesTests`, `FaunarPlayTests` y `FaunarVisualPreview`. La suite completa aprobó 116 casos de reglas. Las pruebas de UI usan el Canvas y las acciones reales; el menú **War & Conquer → Comprobar y mostrar Graybox 3D** ejecuta esa comprobación explícitamente. Las capturas quedan en Library/WarConquer3D.
