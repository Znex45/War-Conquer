...
namespace WarConquer
{
    public static class PrototypeScenario
    {
        ...
        public static string DemoUnit(string leader)=>leader=="FAUNAR"?"charming-mongoose":leader=="SAHRIA"?"dune-cammel":"alligator-revengeful-bite";
        public static void Load(GameManager g)
        {
            ...
            foreach(var player in s.players.Where(p=>!p.inactive))
            {
                ...
                if(g.Catalog.HasDeckDefinitions&&!faunar)hand=fungus?new[]{"terraform","clearing-space","frogpit","micelium-root","firefly-nest","lushroom","alligator-revengeful-bite","bomb"}:new[]{"terraform","clearing-space","sandy-anthill","sandy-ember","dune-cammel","old-mummy","wandering-beast","baby-phoenix"};
                ...
                foreach(var t in home){t.biome=faunar?Biome.Forest:fungus?Biome.Swamp:Biome.Desert;t.setBy=player.id;}
            ...
            if(g.Catalog.HasDeckDefinitions)
            {Spawn(g,0,DemoUnit(s.players[0].leader),a);Spawn(g,1,DemoUnit(s.players[1].leader),b);}
            else
            ...
            if(!g.Catalog.HasDeckDefinitions&&s.players[0].leader=="ZUKGROK")
            {
    ...
    public static class StateValidator
    {
        ...
        {
            if(s==null||s.version!=7||s.rules==null||s.players==null||s.players.Count!=4||s.tiles==null||s.mapPlayers<2||s.mapPlayers>4||s.tiles.Count<50)return "Partida incompatible con las etapas y la puntuación. Inicia una nueva partida.";
            ...
            if(s.pendingChoices==null||s.pendingChoices.Any(c=>c.owner<0||c.owner>3||c.count<1||!ChoiceManager.KnownKind(c.kind)||c.kind!="Discard"&&(c.tileId<0||c.tileId>=s.tiles.Count)))return "Elección pendiente inválida.";
            ...
            foreach(var p in s.players)
            {
                ...
                var all=p.deck.Concat(p.hand).Concat(p.discardPile).Concat(s.pendingChoices.Where(c=>c.owner==p.id&&!string.IsNullOrEmpty(c.returnCardId)).Select(c=>new CardInstance{instanceId=c.sourceId,cardId=c.returnCardId})).ToList();
                ...
                foreach(var group in all.GroupBy(c=>c.cardId))if(group.Count()!=catalog.Copies(p.leader,catalog[group.Key]))return "Distribución de cartas alterada.";
            }
