...
namespace WarConquer
{
    public static class AbilityManager
    {
        ...
        {
            if(source==null||!g.CanAct||source.owner!=g.State.activePlayer||source.abilityUsed||g.IsSleeping(source))return new List<int>();
            var c=g.Data(source);
        ...
        {
            if(p==null||!g.CanAct||p.owner!=g.State.activePlayer||p.abilityUsed||g.IsSleeping(p)||!HasActive(g.Data(p)))return g.Fail("La habilidad no está disponible.");
            ...
            if(c.Has("ExtendBuff"))
            {
                ...
                if(g.State.Active.spores<1||bonus==0)return g.Fail("Requiere 1 Espora y otra unidad adyacente con una mejora temporal de ATQ.");
                g.State.Active.spores--;g.State.tiles[targets[0]].unit.bonusAttack+=bonus;
            }
            if(c.Has("FastNetwork"))TerrainManager.CreateFastRoute(g,targets[0],targets[1],p);
            if(c.Has("StructureDiscount"))g.State.Active.structureDiscount++;
            ...
            p.abilityUsed=true;g.Notify("Habilidad de "+c.name+" resuelta.");return true;
        }
        public static List<int> LeaderTargets(GameManager g)
        {
            if(!g.CanAct||g.State.Active.currentEnergy<g.State.rules.leaderAbilityCost)return new List<int>();
            if(g.State.Active.leader=="SAHRIA")return g.State.tiles.Where(t=>t.owner==g.State.activePlayer&&t.biome==Biome.Desert).Select(t=>t.id).ToList();
            var connected=new HashSet<int>();
            foreach(var route in g.State.fastRoutes.Where(f=>f.owner==g.State.activePlayer))
                foreach(int id in BoardManager.ConnectedBiome(g.State,route.a,g.State.activePlayer,Biome.Forest,Biome.Swamp))connected.Add(id);
            return g.State.tiles.Where(t=>t.unit!=null&&t.unit.owner!=g.State.activePlayer&&t.biome==Biome.Forest&&(connected.Contains(t.id)||t.neighbors.Any(connected.Contains))).Select(t=>t.id).ToList();
        }
        public static bool Leader(GameManager g,IList<int> targets)
        {
            int max=g.State.Active.leader=="SAHRIA"?2:1;
            ...
            g.State.Active.currentEnergy-=g.State.rules.leaderAbilityCost;
            int bonusGroup=g.State.nextId++;
            foreach(int id in targets)
            {
                var t=g.State.tiles[id];
                if(g.State.Active.leader=="ZUKGROK")
                {
                    EffectManager.Poison(g,t.unit,1,g.State.activePlayer);
                    int free=t.neighbors.FirstOrDefault(n=>!g.State.tiles[n].IsOccupied&&!g.State.tiles[n].blocked&&g.State.tiles[n].baseOwner<0,-1);
                    if(free>=0){g.State.tiles[free].owner=g.State.activePlayer;g.Place("espora",g.State.activePlayer,free);}
                }
            ...
            g.Notify("Habilidad de "+g.State.Active.leader+" resuelta.");return true;
        }
    }
}
