...
namespace WarConquer
{
    ...
    public static class GenericCardRules
    {
        ...
        public static string BlockReason(GameManager g,CardData c,CardInstance instance)
        {
            ...
            if(c.IsStructure&&c.health<=0)return "VIDA pendiente de definición para esta estructura.";
            return "";
        }
        ...
        public static void Heal(GameManager g,Piece p,int amount){if(p!=null&&p.health>0&&!FactionCardRules.HealingBlocked(g,p))p.health=Math.Min(g.MaxHealth(p),p.health+amount);}
        public static int ForestHealth(GameManager g,Piece p)=>(g.Data(p).Has("ForestHealth")&&g.State.tiles[p.tileId].biome==Biome.Forest?2:0)+FactionCardRules.TokenStats(g,p);
        public static int MovementAura(GameManager g,Piece p)=>g.Data(p).IsStructure?0:g.Allies(p.owner).Count(x=>g.Data(x).Has("GlobalMovement"))+FactionCardRules.SwampMovement(g,p);
        public static void SyncAuras(GameManager g)
        {
            foreach(var p in BoardManager.Pieces(g.State).ToList())
            {
                ...
                if(!g.IsSleeping(p))
                {
                    // Preserve spent bonus movement when an aura disappears and is later restored.
                    int balance=p.remainingMovement-p.movementAuraDebt+movement-p.movementAura;
                    p.remainingMovement=Math.Max(0,balance);p.movementAuraDebt=Math.Max(0,-balance);
                }
        ...
        {FactionCardRules.OnPlayed(g,p);if(g.Data(p).Has("ForestDraw")&&g.State.tiles[p.tileId].biome==Biome.Forest)DeckManager.Draw(g.State,g.State.players[p.owner],1);}
        ...
        {FactionCardRules.OnKill(g,source,victim);if(source!=null&&source.health>0&&!g.Data(victim).IsStructure&&victim.health<=0&&g.Data(source).Has("KillHeal"))Heal(g,source,g.MaxHealth(source));}
        public static void OnStart(GameManager g,Piece p)
        {
            var c=g.Data(p);var s=g.State;FactionCardRules.OnStart(g,p);
        ...
        public static bool IsActive(CardData c)=>new[]{"MedCamp","RestingCamp","VigilantTower","ForestLeap","WanderingBeast","MushroomToken","FireflyToken"}.Any(c.Has);
        public static List<int> AbilityTargets(GameManager g,Piece p)
        {
            var c=g.Data(p);var s=g.State;var special=FactionCardRules.AbilityTargets(g,p);if(special!=null)return special;
        ...
        {
            var c=g.Data(p);FactionCardRules.Activate(g,p,targets);
