...
namespace WarConquer
{
    public partial class WarConquerController
    {
        ...
        void DrawPendingChoice()
        {
            ...
            GrayboxUI.Text(left,Game.ActingPlayer.isAI?"La IA está resolviendo.":choice.kind=="Discard"?"Selecciona cartas en el panel.":"Selecciona una unidad resaltada en el tablero.",12,195,214,62,14,GrayboxUI.Muted);
        ...
        void ShowDiscardChoice()
        {
            ...
            var player=Game.State.players[choice.owner];discardSelection.RemoveAll(id=>!player.hand.Any(c=>c.instanceId==id));
            ...
            GrayboxUI.Button(body,"CONFIRMAR DESCARTE",28,712,1065,52,()=>{
                var ids=discardSelection.ToArray();CloseModal();ChoiceManager.Resolve(Game,ids);discardSelection.Clear();displayedChoice=null;Render();},GrayboxUI.Green,discardSelection.Count==choice.count);
        }
    }
}
