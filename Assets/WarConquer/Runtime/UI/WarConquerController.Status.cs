...
namespace WarConquer
{
    public partial class WarConquerController
    {
        void DrawScoreboard()
        {
            var s=Game.State;GrayboxUI.Text(inspector,"CONQUISTA · META 10",12,12,300,26,18,GrayboxUI.Ink,FontStyle.Bold);
            for(int i=0;i<4;i++)
            {
                ...
                GrayboxUI.Text(row,control,8,43,290,17,11,GrayboxUI.Muted);
                GrayboxUI.Box(row,"Progreso",0,62,304*Mathf.Clamp01(p.conquestPoints/10f),3,color);
            }
            GrayboxUI.Text(inspector,"N/E/S/O/C: casillas controladas por zona",13,330,296,18,10,GrayboxUI.Muted);
        }
        void DrawSelection()
        {
            var s=Game.State;var box=GrayboxUI.Rect(inspector,"Detalle de selección",12,358,300,274);
            GrayboxUI.Text(box,ModeName(),0,0,209,23,14,GrayboxUI.Muted,FontStyle.Bold);
            GrayboxUI.Button(box,"Cancelar",219,-2,81,26,()=>{ClearAction();Render();});
            ...
            else
            {
                ...
                string info=tile==null?"El centro y cada base enemiga conquistada dan +1 punto por ronda. Gana con 10 puntos o con el último Líder en pie.":"Control: "+(tile.owner<0?"Nadie":"J"+(tile.owner+1))+"\nZona: "+(tile.territory==4?"Centro":"J"+(tile.territory+1))+" · Recurso +"+tile.resource+(tile.specialEffect!=null?"\nTerreno inestable: "+tile.specialEffect.threshold+"+":"");
